...
int main(int argc, char* argv[]) {
    ...
    NetDeviceContainer err = p2p.Install(EndNodes.Get(1), Routers.Get(1)); 
    ...
    wifiDevices = wifi.Install(wifiPhy, wifiMac, Routers);
    ...
    Ipv4InterfaceContainer wifiIface = address.Assign(wifiDevices);
    




    OnOffHelper clientHelper("ns3::TcpSocketFactory", InetSocketAddress(errIface.GetAddress(0), 8080));
    ApplicationContainer clientNode = clientHelper.Install(EndNodes.Get(0));
    clientNode.Start(Seconds(1.0));
    clientNode.Start(Seconds(10.0));

    Address serverSinkAddress(InetSocketAddress(errIface.GetAddress(0), 8080));
